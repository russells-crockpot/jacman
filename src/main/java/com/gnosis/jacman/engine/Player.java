...
public class Player implements Constants{
	...
	protected void moveForward(){
		switch (direction){
		case NORTH: center.y = center.y - MOVEMENT_RATE; break;
		case SOUTH: center.y = center.y + MOVEMENT_RATE; break;
		case EAST: center.x = center.x + MOVEMENT_RATE; break;
		case WEST: center.x = center.x - MOVEMENT_RATE; break;
		}
		changeImageBuffer++;
	...
	public void setInputValues(Board board){
		...
		int c = 0;
		cluster[0].setActivation((Globals.state == ENEMY_HUNTER_STATE)? 1:0);c++;
		int moveOptions = board.getMoveOptions(this);
		cluster[c].setActivation(((moveOptions&NORTH) > 0)? 1:0); c++;
		cluster[c].setActivation(((moveOptions&SOUTH) > 0)? 1:0); c++;
		cluster[c].setActivation(((moveOptions&EAST) > 0)? 1:0); c++;
		cluster[c].setActivation(((moveOptions&WEST) > 0)? 1:0); c++;
		cluster[c].setActivation((direction == NORTH)? 1:0); c++;
		cluster[c].setActivation((direction == SOUTH)? 1:0); c++;
		cluster[c].setActivation((direction == EAST)? 1:0); c++;
		cluster[c].setActivation((direction == WEST)? 1:0); c++;
