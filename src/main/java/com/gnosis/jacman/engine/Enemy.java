...
public class Enemy implements Serializable, Constants {
    ...
    protected void moveForward() {
        switch (direction) {
        case NORTH:
            center.y = center.y - Globals.MOVEMENT_RATE;
            break;
        case SOUTH:
            center.y = center.y + Globals.MOVEMENT_RATE;
            break;
        case EAST:
            center.x = center.x + Globals.MOVEMENT_RATE;
            break;
        case WEST:
            center.x = center.x - Globals.MOVEMENT_RATE;
            break;
            ...
            if ((moveOptions == (NORTH|SOUTH))||(moveOptions == (EAST|WEST))) {
                ...
                for (int i = 0; i < outCluster.getCluster().length; i++) {
                    ...
                    case 3:
                        ...
                        break;
                    default:
                        System.err.println("Iteration in move in enemy went too high."); //consider exception
                    }
    ...
    public void setInputValues(Board board) {
        ...
        int c = 0;
        cluster[c].setActivation((alive)? 1:0);
        c++;
        int moveOptions = board.getMoveOptions(this);
        cluster[c].setActivation(((moveOptions&Globals.NORTH) > 0)? 1:0);
        c++;
        cluster[c].setActivation(((moveOptions&Globals.SOUTH) > 0)? 1:0);
        c++;
        cluster[c].setActivation(((moveOptions&Globals.EAST) > 0)? 1:0);
        c++;
        cluster[c].setActivation(((moveOptions&Globals.WEST) > 0)? 1:0);
        c++;
